...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $new_code = trim($_POST['block_code'] ?? '');
    $block_name = trim($_POST['block_name'] ?? '');
    if ($new_code === '' || $block_name === '') {
        $error = 'Block code and name are required.';
    } else {
        try {
            $stmt = $pdo->prepare("UPDATE blocks SET block_code = ?, block_name = ? WHERE block_code = ?");
            $stmt->execute([$new_code, $block_name, $code]);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h2 mb-0">Edit Block</h1>
    ...
    <div class="card-body">
        ...
        <form method="post" action="">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="block_code" name="block_code" value="<?php echo htmlspecialchars($block_code); ?>" required>
            </div>
            <div class="mb-3">
                <label for="block_name" class="form-label">Block name</label>
                <input type="text" class="form-control" id="block_name" name="block_name" value="<?php echo htmlspecialchars($block_name); ?>" required>
            </div>
