...
$stmt = $pdo->prepare("SELECT * FROM requirements WHERE requirement_id = ?");
$stmt->execute([$id]);
$row = $stmt->fetch();
if (!$row) {
    header('Location: index.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($requirement_name === '') {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE requirements SET requirement_name = ?, department_id = ? WHERE requirement_id = ?");
            $stmt->execute([$requirement_name, $department_id, $id]);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h2 mb-0">Edit Requirement</h1>
    ...
    <div class="card-body">
        ...
        <form method="post" action="">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="requirement_name" name="requirement_name" value="<?php echo htmlspecialchars($requirement_name); ?>" required>
            </div>
            <div class="mb-3">
