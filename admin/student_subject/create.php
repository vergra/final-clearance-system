...
$subject_id = (int)($_POST['subject_id'] ?? ($subjects[0]['subject_id'] ?? 0));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$lrn || !$subject_id || !$school_year_id) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO student_subject (lrn, subject_id, school_year_id) VALUES (?, ?, ?)");
            $stmt->execute([$lrn, $subject_id, $school_year_id]);
            header('Location: index.php?created=1');
            exit;
        } catch (PDOException $e) {
            $error = $e->getCode() == 23000 ? 'This student is already enrolled in this subject for this year.' : 'Failed to save.';
        }
    }
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h2 mb-0">Enroll Student in Subject</h1>
